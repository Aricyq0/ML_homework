...
# spamFileList=spam.get_File_List(".\BayesSpam\data\spam")
...
for fileName in normFileList:
    wordsList.clear()
    for line in open("../data/normal/"+fileName,'rb').read().decode('gbk','ignore'):
            #过滤掉非中文字符
...
for fileName in spamFileList:
    wordsList.clear()
    for line in open("../data/spam/"+fileName,'rb').read().decode('gbk','ignore'):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
...
for fileName in testFileList:
    ...
    wordsDict.clear()
    wordsList.clear()
    for line in open("../data/test/"+fileName,'rb').read().decode('gbk','ignore'):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
